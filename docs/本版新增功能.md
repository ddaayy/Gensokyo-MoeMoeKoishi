...
- 本地文件日志默认关闭，仅启动参数 `run --local-logger=enable` 时写入。
- 文件日志写入 `log/` 下的纯文本文件，`log_suffix_per_mins` 可控制文件时间切分。
- `save_logs`、`log_json_output`、`log_max_age_days`、`log_max_size_mb` 为兼容保留项，当前不控制文件日志启停或轮转。
