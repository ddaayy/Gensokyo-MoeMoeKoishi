...
当 `global_private_to_channel` 开启时，C2C 私聊消息会被转换为群消息格式上报（`message_type: "group"`）。此时群消息事件中会额外携带 `is_private: true` 字段，标识该消息实际来源于私聊，方便后端区分处理。
...
未开启 `global_private_to_channel` 时，私聊消息直接以 `message_type: "private"` 上报，无需此字段。
